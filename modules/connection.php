...
$conn = null;
try{
    $conn = new PDO("mysql:host=".MYSQL_HOSTNAME.";dbname=".MYSQL_DBNAME.";charset=utf8",MYSQL_USERNAME,MYSQL_PASSWD);
...
function ispisiMeni($roditelj){
    ...
    $pripremaPodmenija = $conn->prepare($dohvatiDecuMenija);
...
FROM kategorija k INNER JOIN kategorija_podkategorija kp ON k.kategorija_id = kp.id_kat INNER JOIN podkategorija p ON kp.id_podkat=p.podkat_id WHERE kp.id_kat=:idKat";
    $priprema = $conn->prepare($dohvatiPodkategorije);
